...
export const varifySignUpOtp = asyncHandler(async (req, res) => {
	const { otp,email,password,username,mobile,companyName,role } = req.body;
		...
		const userDetails = {
		 ...
		 try{
			...
			console.log({user})
				...
				const createdUser = await User.findByIdAndUpdate(user[0]._id,{company_id:company[0]._id},{session,new:true})
