...
export const createChatService = asyncHandler(async (data) => {
	...
	const fullChat = await Chat.findOne({ _id: createdData._id }).populate('users');
	return fullChat;
});
...
export const getExistChat = asyncHandler(async (userId, inUserId) => {
	var result = Chat.find({
		isGroupChat: false,
		$and: [{ users: { $elemMatch: { $eq: inUserId } } }, { users: { $elemMatch: { $eq: userId } } }]
	})
		.populate('users')
...
export const searchUserService = asyncHandler(async (userId, keyword) => {
