			...
			<AppBar position="static">
				<StyledToolbar>
					...
					<MenuBox sx={{ display: { xs: 'none', md: 'flex' } }}>
						{menu.map((v) => {
							return (
								<Typography key={v.name} sx={{ cursor: 'pointer', color: 'black' }}>
									{v.name}
								</Typography>
							);
						})}
					...
					<MenuIcon sx={{ color: 'black', display: { xs: 'flex', md: 'none' } }} />
					...
					<Box
						...
						<Button variant="contained" onClick={()=>navigate('/signup')} sx={{ width: '140px', height: '45px', fontWeight: 600 }}>
							Start free 
						</Button>
						<Button
							variant="outlined"
